              ...
              <Breadcrumb>
                <BreadcrumbList>
                  {route.breadcrumbs.map((item, index) => {
                    ...
                    const breadcrumbHref = getBreadcrumbHref(
                      route.breadcrumbs,
                      index
                    )
                      ...
                      <Fragment key={`${item}-${index}`}>
                        ...
                        <BreadcrumbItem>
                          {isLast ? (
                            ...
                            <BreadcrumbLink render={<Link href={breadcrumbHref} />}>
                              {item}
                            </BreadcrumbLink>
                          ) : (
...
function getBreadcrumbHref(breadcrumbs: readonly string[], index: number) {
  if (index < 0 || index >= breadcrumbs.length - 1) {
    return null
  }

  const prefix = breadcrumbs.slice(0, index + 1)

  const matchedRoute = Object.values(appRoutes).find((route) => {
    if (route.breadcrumbs.length !== prefix.length) {
      return false
    }

    return route.breadcrumbs.every((item, itemIndex) => item === prefix[itemIndex])
  })
